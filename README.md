...
The deadline for Homework 4 is Friday, October 6, 6pm. The late submission deadline is Friday, October 13, 4pm.
...
available at
`https://github.com/nyu-popl-fa23/hw04-<YOUR-GITHUB-USERNAME>`.
Note that this may take a few minutes.

* Open a browser at `https://github.com/nyu-popl-fa23/hw04-<YOUR-GITHUB-USERNAME>` with your Github username inserted at the appropriate place in the URL.
* Choose a place on your computer for your homework assignments to reside and open a terminal to that location.
* Execute the following git command: <br/>
  ```bash
  git clone https://github.com/nyu-popl-fa23/hw04-<YOUR-GITHUB-USERNAME>.git hw04
  cd hw04
  ```
...
The problems that you you should solve for this assignment are described in the file `hw04.pdf`.

The assignment consists of a coding part (Problem 1) and a non-coding part (Problem 2).
...
2. The code template for solving Problem 2 is provided in the file 
   ```
   src/main/scala/popl/hw04.scala 
   ``` 
   ...
   expressions in `hw04.scala` with your code for the corresponding
...
The parsed expression is then passed to your implementation of the interpreter (the function `eval` defined in `hw04.scala`). For instance, to run your interpreter on the file `testjs/test01_arith.js`, you can execute the following command from the command line:
...
```
sbt:hw04> run testjs/test01_arith.js
```
...
Alternatively, you can run the interpreter directly from inside the IDE. To do so, right-click the file `hw04.scala` and select "More Run/Debug" and then "Modify Run Configuration". You can provide the command line arguments to the interpreter in the text field labeled "Program arguments". Enter e.g. "testjs/test01_arith.js" in this field and click "OK". When you now right-click on the file `hw04.scala` and select "Run 'hw04'", then the interpreter will be executed with the specified command line arguments.
